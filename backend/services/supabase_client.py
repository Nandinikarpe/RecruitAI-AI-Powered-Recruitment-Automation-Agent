...
def _project_url() -> str:
    """PostgREST expects the project URL without a trailing slash."""
...
def get_supabase() -> Client:
    global _client
    if _client is None:
        _client = create_client(_project_url(), settings.supabase_key)
    return _client
...
def get_supabase_admin() -> Client:
    ...
    if key:
        return create_client(_project_url(), key)
    return get_supabase()
